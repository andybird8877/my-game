...
for (const affinity of affinities) {
  for (const weight of weights) {
    for (const cls of classes) {
      ...
      CHARACTERS.push({
        id:       _id++,
        name:     isCairan ? 'Cairan Vex' : isMourne ? 'Mourne' : isVael ? 'Vael Solace' : isWrack ? 'Wrack' : isHarrox ? 'Harrox' : `${cap(affinity)} ${cap(weight)} ${cap(cls)}`,
        portrait:  isCairan ? '/portraits/cairan-vex.png' : isMourne ? '/portraits/mourne.png' : isVael ? '/portraits/vael-solace.png' : isWrack ? '/portraits/wrack.png' : isHarrox ? '/portraits/harrox.png' : null,
        hasDodge:  isCairan,
        hasMourne: isMourne,
        hasVael:   isVael,
        hasWrack:  isWrack,
